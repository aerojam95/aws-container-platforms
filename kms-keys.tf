...
module "ecs_cluster_logs_kms_key" {
  source      = "terraform-aws-modules/kms/aws"
  version     = "~> 1.5.0"
  description = "${local.name} AWS CloudWatch ECS cluster logs Encryption KMS Key"
  key_owners  = ["${data.aws_caller_identity.current.arn}"]
  ...
  aliases = ["${local.name}-cw-ecs-cluster-logs-kms-key"]

  tags = merge(
    { "Name" = "${local.name}-cw-ecs-cluster-logs-kms-key" },
    local.resource_tags
  )
...
module "ecs_service_logs_kms_key" {
  source      = "terraform-aws-modules/kms/aws"
  version     = "~> 1.5.0"
  description = "${local.name} AWS CloudWatch ECS service logs Encryption KMS Key"
  key_owners  = ["${data.aws_caller_identity.current.arn}"]

  key_statements = [
    {
      sid = "CWECSServicerLogs"
      actions = [
        "kms:Encrypt*",
        "kms:Decrypt*",
        "kms:ReEncrypt*",
  ...
  aliases = ["${local.name}-cw-ecs-service-logs-kms-key"]

  tags = merge(
    { "Name" = "${local.name}-cw-ecs-service-logs-kms-key" },
    local.resource_tags
  )
...
###############################################################################
# ALB logging S3 bucket KMS key
###############################################################################

module "alb_logs_s3_bucket_kms_key" {
  source      = "terraform-aws-modules/kms/aws"
  version     = "~> 1.5.0"
  description = "${local.name} ALB logs S3 bucket Encryption KMS Key"
  key_owners  = ["${data.aws_caller_identity.current.arn}"]
  ...
  aliases = ["${local.name}-alb-logs-s3-bucket-kms-key"]

  tags = merge(
    { "Name" = "${local.name}-alb-logs-s3-bucket-kms-key" },
    local.resource_tags
  )
