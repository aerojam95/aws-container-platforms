...
module "s3_bucket_alb_logs" {
  ...
  bucket              = format("${local.name}-s3-bucket-alb-logs")
  acl                 = "log-delivery-write"
  allowed_kms_key_arn = module.alb_logs_s3_bucket_kms_key.key_arn

  server_side_encryption_configuration = {
    rule = {
      apply_server_side_encryption_by_default = {
        kms_master_key_id = module.alb_logs_s3_bucket_kms_key.key_arn
  ...
  logging = {
    target_bucket = module.s3_bucket_alb_logs.s3_bucket_id
    target_prefix = "ecs-cluster-alb-logs/"
  }

  lifecycle_rule = [
    {
      id      = "ecs-cluster-alb-log"
      enabled = true
      prefix  = "ecs-cluster-alb-logs/"
      tags = {
        rule      = "ecs-cluster-alb-logs"
        autoclean = "true"
      }
      transition = [
        {
  ...
  tags = merge(
    { "Name" = format("${local.name}-s3-bucket-alb-logs") },
    local.resource_tags
  )
}
